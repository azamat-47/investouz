    ...
    return (
        <>
            <div className="col-xl-4 col-md-6 mt-4  ">
                <div className=" shadow m-1 px-3 py-4 border h-full rounded min-h-82 bg-white ">
                    <div className="flex items-center gap-2">
                        ...
                        <p className="m-0! opacity-60">
                            {business.titleInfo.split(" ").slice(0, 5).join(" ")}
                            {business.titleInfo.split(" ").length > 5 && " ..."}

                        </p>
                    </div>
                    <h2 className="text-xl font-bold mb-0 text-gray-800">{business.companyName}</h2>
                    <p className="text-sm  text-blue-700">#{business.companyType}</p>
                    <p className="opacity-80 ">
                        {business.info.split(" ").slice(0, 10).join(" ")}
                        {business.info.split(" ").length > 10 && " ..."}
                    </p>
                    <img src={business.image} className="h-40 w-full" alt={business.companyName} />
                    <p className="text-sm text-gray-600 flex mt-1 gap-1 mb-0">

                        <MapPinCheck size={19} strokeWidth={1.5} absoluteStrokeWidth />
                        {business.locationPreference}
                        <span>
                    ...
                    <div className="flex mt-3 grid-cols-2">
                        <div className="border-r-1 w-1/2 ">
                            <p className="card_type_info opacity-70 mb-0 items-center">
                                <span className="text-amber-800 font-bold mr-1!">{business.investmentVolume}</span>
                                biznes sotish uchun!
                                <Info size={10} strokeWidth={2.5} color="#4B5563" className="inline " />
                            </p>
                            <p className="text-xl font-bold text-blue-900 mb-0"> <span className="text-orange-400 text-sm font-bold">{business.investmentValyuta}</span>  {business.investmentSize.toLocaleString()} </p>
                        </div>
